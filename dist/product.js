...
function renderWatch(slug, updateUrl = true) {
  ...
  $("#watch-specs").innerHTML = currentWatch.specs
  ...
  if (updateUrl)
    history.replaceState(
      { watch: currentSlug },
      "",
      `/product.html?watch=${currentSlug}`,
    );
}
...
$("#watch-add").addEventListener("click", (event) => {
  ...
  if (line) line.qty += 1;
  else cart.push({ id: currentWatch.id, qty: 1 });
...
window.addEventListener("popstate", () =>
  renderWatch(
    new URLSearchParams(location.search).get("watch") || "azur",
    false,
  ),
);
